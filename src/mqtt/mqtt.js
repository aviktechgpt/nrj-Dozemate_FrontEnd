...
// Subscribe to default topic (backward compatibility)
const subscribeToDefaultTopic = () => {
  const DEFAULT_TOPIC = '/test/topic';
  console.log(`No device ID provided. Subscribing to default topic: ${DEFAULT_TOPIC}`);
  
  client.subscribe(DEFAULT_TOPIC, (err) => {
    if (!err) {
      console.log(`Successfully subscribed to default topic: ${DEFAULT_TOPIC}`);
    } else {
      console.error(`Failed to subscribe to default topic: ${err}`);
    }
  });
};

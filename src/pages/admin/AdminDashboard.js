  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        // Get organizationId using the dedicated API
        const orgRes = await fetch("https://admin.dozemate.com/api/user/user/organization-id", {
          headers: { Authorization: `Bearer ${token}` },
        });
        const orgData = await orgRes.json();
        if (!orgRes.ok || !orgData.organizationId) {
          setError("Failed to get admin's organization.");
          setLoading(false);
          return;
        }
        const orgId = orgData.organizationId;
        // Use the admin-only API to get all users in the organization
        const usersRes = await fetch(
          `https://admin.dozemate.com/api/manage/users/
organization/${orgId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
